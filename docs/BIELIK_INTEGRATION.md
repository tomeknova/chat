# BIELIK_INTEGRATION.md — roadmap v2.5 (lokalny LLM w Laravel 12)
...
> **v2.5:** ZAIMPLEMENTOWANE i wypchnięte na `main` (Fazy 1–6 w wersji v1) — patrz „STAN IMPLEMENTACJI".
> Async 202/polling z R = N/A dla synchronicznego Livewire. Semafor / heartbeat+digest / bge-m3 = świadomie odłożone.
...
## STAN IMPLEMENTACJI (2026-06-22) — zbudowane, testy zielone, żywy e2e na Bieliku OK

Mapowanie Faz → kod (commity na `main`):
- **Faza 1** (seam + adaptery + grounding-in-attempt Q): `99ea3ce` — moduł `App\AskDocs` (`ChatModel`, `AnswerUnitSelector`, `OllamaChatModel`, `OpenRouterChatModel`, `FailoverAnswerUnitSelector`, `GroundingValidator`), `config/askdocs.php`.
- **Faza 2** (endpoint po NAZWIE + allowlista CIDR anty-SSRF): `339645d` — `DnsEndpointResolver` + `EndpointAllowlist` (pin do IP, fail-closed). Heartbeat HMAC + weryfikacja digestu = ODŁOŻONE.
- **Faza 3** (breaker + deadline): `99ea3ce` — `CircuitBreaker` per-provider (na razie JEDEN, nie rozdzielony Transport/Quality) + deadline-aware timeouty (per-provider timeout + budżet).
- **Faza 4** (retrieval mierzony): `ab18973` — `LexicalRetriever`; `83bd436` — `chat:eval` (Recall@8=100%, @5=75%, @3=67%). bge-m3 = ODŁOŻONE (lexical wystarcza przy top_k=8).
- **Faza 5** (współbieżność): semafor fail-closed = ODŁOŻONE (chronią throttle + dzienny budżet + breaker + timeouty; poprawny semafor = per-provider/kolejka, gdy realny współbieżny ruch).
- **Faza 6** (observability): `1bbf8eb` — rezerwacja CAS + lease takeover + telemetria `metadata.attempts[]` + migracja; `6c943a8` — `status`/`attempts` w Filament. Pełna lista pól `metadata` = backlog.
- **Faza 7** (suggestions/recovery): `2923304` — startery (config) pod powitaniem + recovery przy abstynencji z intentów najbliższych jednostek; akcja `Chat::ask` + chipy. Źródło zawsze odpowiadalne (intenty), nigdy generowane modelem na runtime.

UX poza Fazami: render markdown odpowiedzi (`3495c37`), opisowy label źródła = nagłówek jednostki (`6e15251`), strefa `Europe/Warsaw` (`abcabf0`), favicon SVG (`8fb73ae`). Testy hermetyczne — routing AskDocs zapięty w `phpunit.xml`.
>
...
### Faza 7 — Suggestions / recovery (ZAIMPLEMENTOWANE — `2923304`, grounded)
Prowadzenie niepewnego usera klikalnymi podpowiedziami. **Źródło = intenty korpusu** (odpowiadalne z definicji) — nigdy free-generation pytań przez model na runtime (ryzyko podpowiedzi bez pokrycia + koszt + łamie verbatim).
- **Startery** (pusty czat): 3 pytania — kuratorowane w `config/chat.php` lub top intenty.
- **Recovery** (przy abstynencji domenowej / `out_of_scope`, NIE przy błędzie technicznym): weź intenty najbliższych kandydatów retrievera (te, które model widział) → 2–3 „Może chodziło Ci o:"; brak kandydatów (off-topic) → startery. To „weryfikacja z korpusem → przy braku trafienia zaproponuj pytanie w dobrym kontekście".
- **UI:** klikalne chipy + akcja `Chat::ask(string $q)` (ustawia `question` + `sendMessage`); współdzielone przez startery i recovery.
- **Model:** opcjonalnie Bielik OFFLINE (przy `chat:build-corpus`) do ładniejszego frazowania/wyboru intentów — lekkie, lokalne, darmowe. GPT zbędny (większa „inteligencja" nie kupuje groundingu; mądrzejszy model = pewniej brzmiące, ale wciąż potencjalnie nieodpowiadalne pytania).
- **Ograniczenia:** jakość recovery zależy od recall retrievera (luka słownikowa → przyszły `bge-m3`); intenty są page-level → podpowiedzi ogólne do czasu granularności docs (ten sam lever co jakość odpowiedzi).
- **Bramka:** każda podpowiedź jest klikalna i kończy się ugruntowaną odpowiedzią (pochodzi z intentów), nie kolejną abstynencją.
